...
const svgPath = join(root, "public/images/partners/target.svg");
const tempDir = join(root, ".tmp-target-logo-video");
const pngPath = join(tempDir, "target-logo.png");
const outputDir = join(root, "public/videos");
const outputPath = join(outputDir, "target-logo-dots.mp4");

const width = 1920;
const height = 1080;
const sourceWidth = 390;
const sourceHeight = 497;
const fps = 30;
const duration = 4.8;
...
if (mask.status !== 0 || !mask.stdout?.length) {
  throw new Error("Could not rasterize Target SVG mask.");
}
...
if (!particles.length) {
  throw new Error("No Target logo particles were sampled.");
}
